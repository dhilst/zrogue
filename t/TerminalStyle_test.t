...
subtest 'constructor allows omitted keys' => sub {
    my $style = TerminalStyle::new(-fg => 0xffffff);
...
subtest 'constructor validates keys and values' => sub {
    dies_ok { TerminalStyle::new(foo => 1) } 'unknown key dies';
    dies_ok { TerminalStyle::new(-fg => 0x1000000) } 'fg out of rgb range dies';
    dies_ok { TerminalStyle::new(-bg => -2) } 'bg below sentinel dies';
    dies_ok { TerminalStyle::new(-attrs => 1.5) } 'attrs must be integer';
};
...
subtest 'from_hashref normalizes existing style hash' => sub {
    my $style = TerminalStyle::from_hashref({
        -fg => 0xaabbcc,
        -attrs => 3,
    });
...
subtest 'from_hashref rejects invalid input' => sub {
    dies_ok { TerminalStyle::from_hashref([]) } 'non hashref dies';
    dies_ok { TerminalStyle::from_hashref({ nope => 1 }) } 'invalid style hash dies';
};
